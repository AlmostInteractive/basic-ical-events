...
const parseParameters = (p: any): any => {
  const out: any = {};
...
const storeValueParameter = (name: string | number) => {
  return (value: any, curr: { [x: string]: any[]; }) => {
...
const storeParameter = (name: string) => {
  return (value: string | undefined, parameters: string | any[], curr: any) => {
...
const addTZ = (dt: any, parameters: any) => {
...
const geoParameter = (name: string) => {
  return (value: string, parameters: any, curr: { [x: string]: { lat: number; lon: number; }; }) => {
    //@ts-ignore
...
const categoriesParameter = (name: string) => {
  const separatorPattern = /\s*,\s*/g;
  return (value: string, parameters: any, curr: { [x: string]: string | any[]; }) => {
    //@ts-ignore
...
const exdateParameter = (name: string) => {
  return (value: string, parameters: any, curr: { [x: string]: { [x: string]: any; }; }) => {
    const separatorPattern = /\s*,\s*/g;
    curr[name] = curr[name] || [];
...
const recurrenceParameter = (name: string) => {
  return dateParameter(name);
};

const addFBType = (fb: { type?: any; }, parameters: any) => {
  const p = parseParameters(parameters);
...
const freebusyParameter = (name: string) => {
  return (value: string, parameters: any, curr: { [x: string]: any[]; }) => {
    ...
    dateParameter('end')(parts[1], parameters, fb);

    return curr;
  };
};

const getLineBreakChar = (string: string | string[]) => {
...
const objectHandlers: any = {
  ...
  END(value: string, parameters: any, curr: { rrule: string; start: any; }, stack: any) {
    ...
    //@ts-ignore
    const originalEnd = (component: string, parameters_: any, curr: {
      [x: string]: any;
      end: Date;
      datetype: string;
      start: any;
      ...
      uid: string | number;
      recurrenceid: { toISOString: () => string; } | undefined;
    }, stack: any[]) => {
...
export const handleObject = (name: string, value: any, parameters: any, ctx: any, stack: string | any[], line: any) => {
...
export const parseLines = (lines: string | any[], limit: number, ctx?: Context | undefined, stack?: never[], lastIndex?: number) => {
  ctx = ctx || {};
  stack = stack || [];
...
export const parseICS = (string: string) => {
...
const getErrorMessage = (unknownError: unknown) => {
  if (!axios.isAxiosError(unknownError)) {
    return `Non AxiosError: ${(unknownError as Error).message}`;
  }
...
export const fetchFromFile = async (filename: string): Promise<any> => {
